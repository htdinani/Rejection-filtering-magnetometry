...
def rflt(phi): 
    th=-0.10 #The threshold for the restarting part of the algorithm
    cntmax=3 # cntmax is the max of "cnt" in the restarting part of the algorithm. In other words "cntmax" number of measurements is done and if the estimated phase "mu" is not found to be accurate the algorithm restarts by setting "mu" to initial mu and "sigma" to initial sigma 
    nums=1000 #num of samples drawn from the Gaussian
    nm=50 #a limit that restricts the number of measurements. However the number of measurements could go beyond 100 because if the estimate is not accurate the counter of the "while" loop initializes. 
    ...
    sigmav=[] #an array to keep track of the variances
    ...
    while n1 < nm:
        ...
        #p0=0.5*(1.0+vis*np.cos((t*phi-theta)/tau))
        ...
        #pps=0.5*(1.0+(-1.0)**u1*vis*np.cos(t*phis/tau-theta/tau)) # The probability that the drawn samples "phis" gives the measurement result "u1"
        pps=0.5*(1.0+(-1.0)**u1*vis*np.cos(t*(phis/tau)-theta))
        maxpps=np.amax(pps)
        ...
        if dv>=th and cnt>=cntmax:
            ...
            if ut==0: # Verifies that the estimate i.e., "mu" is accurate 
                ...
                cnt=0
                sigma=sigmav[n1-1]#100*np.amin(sigmav)
                mu=muv[n1-1]#[np.argmin(sigmav)]
                #n1=1
        else:
            cnt=cnt+1
        #########################################################
        t1=np.ceil(1.25/np.sqrt(sigma)) #This calculates the interaction time for the next measurement based on the current variance "sigma".
        t=np.minimum(t1*tau,T2) #If the interaction time is bigger than the decoherence time we choose t=T2
        theta=np.random.normal(mu,np.sqrt(sigma))*t1 #The controlled phase for the next measurement
        n1=n1+1
